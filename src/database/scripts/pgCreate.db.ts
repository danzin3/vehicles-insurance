...
async function enviroment() {
  ...
  switch (flag) {
    ...
    default:
      throw {
        message: 'Creation flag (dev or test) is necessary to this action',
      };
  }
}

async function startup() {
  logger.genericLog('--> Creating Database...');
  try {
    ...
    logger.genericLog('--> Executing the creation query command...');
    await client.query(`CREATE DATABASE "${databaseName}" OWNER "postgres";`);
    await client.end();
    logger.genericLog(
      `--> Creation of database ${databaseName} was successfully Done!`,
    );
    process.exit(0);
  } catch (exception) {
